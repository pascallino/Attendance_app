...
class Workday(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    workdayid = db.Column(db.String(255), unique=True, nullable=True)
    workday_date = db.Column(db.DateTime, nullable=True)
    start_time = db.Column(db.DateTime, nullable=True)
    end_time = db.Column(db.DateTime, nullable=True)
    work_status = db.Column(db.String(255), nullable=True)
    company_id = db.Column(db.String(255), db.ForeignKey('company.companyid'), nullable=False)
    userid = db.Column(db.String(128), db.ForeignKey('user.userid'), nullable=False)
    worktransactions = db.relationship('Worktransaction', backref='workday', lazy=True)
  
    #confirm = db.Column(db.Boolean, default=False)
    
    def __init__(self, workday_date, start_time, end_time, work_status, company_id, userid):
        self.workdayid = str(uuid4())
        self.workday_date = workday_date
...
class Worktransaction(db.Model):
    ...
    sign_in_status = db.Column(db.String(255), nullable=True)
    sign_in_reasons = db.Column(db.Text, nullable=True)
    sign_out_time = db.Column(db.DateTime, nullable=True)
    sign_out_status = db.Column(db.String(255), nullable=True)
    sign_out_reasons = db.Column(db.Text, nullable=True)
    is_approved = db.Column(db.Boolean, default=False)
    approved_by = db.Column(db.String(255), nullable=True)
    Total_hours_worked = db.Column(db.Integer, default=0)
    userid = db.Column(db.String(128), db.ForeignKey('user.userid'), nullable=False)
    workdayid = db.Column(db.String(128), db.ForeignKey('workday.workdayid'), nullable=False)
    
    def __init__(self, Date, sign_in_time, time_diff_from_starttime,
        ...
        self.sign_in_status = sign_in_status
        self.sign_in_reasons = sign_in_reasons
        self.sign_out_time = sign_out_time
        self.sign_out_status = sign_out_status
        self.sign_out_reasons = sign_out_reasons
...
class User(UserMixin, db.Model):
    ...
    workdays = db.relationship('Workday', backref='user', lazy=True)
    worktransactions = db.relationship('Worktransaction', backref='user', lazy=True)
